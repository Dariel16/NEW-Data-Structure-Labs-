...
public class LinkedPositionalList<E> implements PositionalList<E> {
	...
	LinkedPositionalList list=this;  //
	...
	private  class DNode<E> implements Position<E> { 
		...
		private LinkedPositionalList<E> getList(){
			return list;
		}
	}
	...
	private DNode<E> validate(Position<E> p) throws IllegalArgumentException { 
		try { 
			...
			if (dp.getList()!= this ) //validar la lista
	...
	public class PositionalListElementsBackwardIterator implements Iterator<E>{
		...
		public E next() {
			if(!hasNext()){
				throw new NoSuchElementException(" no aditional elements");
			}
			return posIterator.next().getElement();
		}
	...
	public class BackwardElementIterator implements Iterator<E>{
		private DNode<E> cursor = trailer.getPrev(), recent = null;
